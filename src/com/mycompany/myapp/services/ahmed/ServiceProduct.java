...
public class ServiceProduct {
    ...
    public Produit parseOneProduct(String jsonText) {
            Produit p = new Produit();
        ...
        try {
            ...
            //int idc=Integer.parseInt(obj.get("cat").toString());
            Map<String, Object> categorie = (Map) productsListJson.get("cat");
            // Map<String, Object> cat = (Map) categorie.get("id");
            p.setId_Categorie((int) Float.parseFloat(categorie.get("id").toString()));
            p.setId_Produit((int) id);
            // p.setId_Categorie(idc);
            p.setNom_Produit(productsListJson.get("nom").toString());
            p.setTaille_Produit(productsListJson.get("size").toString());
            p.setEtat_Produit(productsListJson.get("etat").toString());
            //p.setId_Categorie(((int)Integer.parseInt(obj.get("cat").toString())));
            p.setPrix_Produit(((int) Float.parseFloat(productsListJson.get("prix").toString())));
            p.setQuantite_Totale(((int) Float.parseFloat(productsListJson.get("qte").toString())));
            p.setUrl(productsListJson.get("url").toString());
    ...
    public ArrayList<Produit> getAllProducts() {
        String url = "http://localhost:81/TGTOf/web/app_dev.php" + "/store/ApiProduit/all";
        req.setUrl(url);
        req.setPost(false);
        req.addResponseListener(new ActionListener<NetworkEvent>() {
    ...
    public Produit getOProducts(int i) {
        ...
        req.addResponseListener(new ActionListener<NetworkEvent>() {
            ...
            @Override
            public void actionPerformed(NetworkEvent evt) {
                Oneprod = parseOneProduct(new String(req.getResponseData()));
                req.removeResponseListener(this);
